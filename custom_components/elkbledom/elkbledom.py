...
class BLEDOMInstance:
    ...
    def get_effect_speed_cmd(self, value: int):
        if self._effect_speed_cmd is None:
            # Protocol: [0x7e, length, cmd_type=0x02, speed, padding, padding, padding, checksum, 0xef]
            return [0x7e, 0x04, 0x02, int(value), 0xff, 0xff, 0xff, 0x00, 0xef]
    ...
    def get_effect_cmd(self, value: int):
        if self._effect_cmd is None:
            # Protocol: [0x7e, length, cmd_type=0x03, effect, sub_param, padding, padding, checksum, 0xef]
            return [0x7e, 0x05, 0x03, int(value), 0x03, 0xff, 0xff, 0x00, 0xef]
    ...
    def get_color_temp_cmd(self, warm: int, cold: int):
        if self._color_temp_cmd is None:
            # Protocol: [0x7e, length, cmd_type=0x05, sub_cmd=0x02, warm, cold, 0xff, checksum=0x08, 0xef]
            return [0x7e, 0x06, 0x05, 0x02, int(warm), int(cold), 0xff, 0x08, 0xef]
    ...
    async def set_color(self, rgb: tuple[int, int, int]):
        ...
        # Protocol: [0x7e, length, cmd_type=0x05, sub_cmd=0x03, R, G, B, checksum=0x10, 0xef]
        await self._write([0x7e, 0x07, 0x05, 0x03, rr, gg, bb, 0x10, 0xef])
        self._rgb_color = rgb
    ...
    async def set_brightness(self, intensity: int):
        # Protocol: [0x7e, length, cmd_type=0x01, brightness, 0xff, padding, padding, checksum, 0xef]
        await self._write([0x7e, 0x04, 0x01, int(intensity*100/255), 0xff, 0xff, 0xff, 0x00, 0xef])
        self._brightness = intensity
    ...
    async def set_rgbw_channels(self, r_on: bool = True, g_on: bool = True, b_on: bool = True, w_on: bool = True, mode: int = 0):
        ...
        """
        # Build bitmask - R controls the RGB group via bits 5-7
        rgb_w_bits = 0
        if r_on:
        ...
        if mode == 0 or mode == 1:  # ALL or RGB mode
            ...
            state = 0x01 if w_on else 0x00
        elif mode in (3, 4):  # CT or Laser mode
